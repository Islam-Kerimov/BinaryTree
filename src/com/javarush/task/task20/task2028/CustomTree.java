...
public class CustomTree extends AbstractList<String> implements Cloneable, Serializable {
    ...
    public boolean add(String s) {
        ...
        if (root.isAvailableToAddChildren()) {
            ...
            for (Entry<String> stringEntry : list) {
                if ((temp = stringEntry).isAvailableToAddChildren()) {
    ...
    public String getParent(String s) {
        for (Entry<String> stringEntry : list) {
            if (s.equals(stringEntry.elementName)) {
                Entry<String> parent = stringEntry.parent;
    ...
    public boolean remove(Object o) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (s.equals(list.get(i).elementName)) {
                cleanParentConnect(list.get(i).parent, s);
                elements = removeListDeleteElements(list, list.get(i).elementName);
                cleanListTree(list, elements);
                break;
            }
        }
        return true;
    ...
    public List<String> removeListDeleteElements(List<Entry<String>> list, String firstElement) {
        List<String> elements = new ArrayList<>();
        elements.add(firstElement);
        for (int iter = 0; iter < elements.size(); iter++) {
            for (Entry<String> stringEntry : list) {
                if (elements.get(iter).equals(stringEntry.elementName)) {
                    if (!stringEntry.availableToAddLeftChildren) {
                        elements.add(stringEntry.leftChild.elementName);
                    }
                    if (!stringEntry.availableToAddRightChildren) {
                        elements.add(stringEntry.rightChild.elementName);
                    }
                    break;
                }
            }
    ...
    public void cleanListTree(List<Entry<String>> list, List<String> elements) {
        for (String element : elements) {
            for (int j = 1; j < list.size(); ) {
                if (element.equals(list.get(j).elementName)) {
                    list.get(j).parent = null;
                    list.get(j).leftChild = null;
                    list.get(j).rightChild = null;
                    list.remove(j);
                    break;
                } else {
                    j++;
    ...
    private void cleanParentConnect(Entry<String> parent, String element) {
        if (parent.leftChild != null && parent.leftChild.elementName.equals(element)) {
            parent.leftChild = null;
        } else if (parent.rightChild != null && parent.rightChild.elementName.equals(element)){
